...
export async function handleRequest(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	if (!serverPage) {
		return generateResponse(request, 404, 'Cannot find API endpoint');
	}
	...
	if (!allowedMethods.includes(method as RequestMethod)) {
		return generateResponse(request, 405, `Method ${method} not allowed`);
	}
		...
		for (const [key, value] of Object.entries(params)) {
			if (typeof data[key] === 'undefined') {
				if ((typeof value === 'string' && value.endsWith('!')) || typeof value === 'object') {
					return generateResponse(request, 400, `Missing required parameter ${key}`);
				}
			} else {
				if (typeof value === 'object') {
					if (typeof data[key] !== 'object') {
						return generateResponse(request, 400, `Invalid type for parameter ${key}`);
					}
	...
	return new Promise((resolve, reject) => {
		...
		try {
			...
			console.error(error);
			return generateResponse(request, 500, 'Internal server error: ' + error.message || error);
		}
		if (response instanceof Promise) {
			response
				.then((res) => {
					resolve(res || generateResponse(request, 204, 'No Content'));
				})
				.catch((error) => {
					console.error(error);
					resolve(generateResponse(request, 500, 'Internal server error: ' + error.message));
				});
		}
		if (response instanceof Response) {
			resolve(response);
		}
		resolve(generateResponse(request, 204, 'No Content'));
	});
}

export function generateResponse(request: Request, code: number, data: string | { [key: string]: any }): Response {
